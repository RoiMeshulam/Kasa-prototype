...
const InsertBottleScreen: React.FC<Props> = ({
  ...
  return (
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: 'white', paddingHorizontal: 24 }}>
      <Text style={{ fontSize: 22, fontWeight: 'bold', marginBottom: 8 }}>הכנס בקבוק למכונה</Text>
      ...
      <Text style={{ fontSize: 18, marginTop: 8 }}>מאזן סריקה: ₪{balance.toFixed(2)}</Text>
      ...
      {current ? (
        <View style={{ alignItems: 'center', marginTop: 16 }}>
          ...
          <Text style={{ fontSize: 16 }}>שווי ליחידה: ₪{current.price.toFixed(2)}</Text>
          <Text style={{ fontSize: 16 }}>עד כה הוכנסו: {current.quantity} יח׳</Text>
        </View>
      ) : (
        <Text style={{ color: '#6b7280', marginTop: 16 }}>אין בקבוק פעיל כרגע</Text>
      )}

      <View style={{ width: '100%', marginTop: 28, gap: 12 }}>
        <TouchableOpacity
          style={{ backgroundColor: '#000', paddingVertical: 12, borderRadius: 12, opacity: current ? 1 : 0.6 }}
          onPress={onInsert}
          disabled={!current}
        >
        ...
        <TouchableOpacity
          style={{ backgroundColor: '#f59e0b', paddingVertical: 12, borderRadius: 12 }}
        ...
        <TouchableOpacity
          style={{ backgroundColor: '#ef4444', paddingVertical: 12, borderRadius: 12 }}
